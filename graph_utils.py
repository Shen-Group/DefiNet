...
def local_mae_indices(pos_u, mask, cutoff=3):
    dist = torch.cdist(pos_u[mask != 0], pos_u).squeeze()
    neighbor = torch.where(dist < cutoff)[-1].unique()
    neighbor_mask = torch.zeros_like(mask)
    neighbor_mask[neighbor] = 1
    final_mask = ((mask != 2) * neighbor_mask).bool()
    indices = torch.where(final_mask == True)[0]
