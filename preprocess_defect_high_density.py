...
if __name__ == "__main__":
    ...
    for materials_id in ['BP_spin_500', 'GaSe_spin_500', 'hBN_spin_500', 'InSe_spin_500', 'MoS2_500', 'WSe2_500']:
        ...
        cif_root = os.path.join(materials_root, 'CIF')
        ...
        if materials_id == 'WSe2_500':
            unit_cell = read(os.path.join(data_root, 'WSe2.cif'))
        elif materials_id == 'MoS2_500':
            unit_cell = read(os.path.join(data_root, 'MoS2.cif'))
        elif materials_id == 'InSe_spin_500':
            unit_cell = read(os.path.join(data_root, 'InSe.cif'))
        elif materials_id == 'hBN_spin_500':
            unit_cell = read(os.path.join(data_root, 'BN.cif'))
        elif materials_id == 'GaSe_spin_500':
            unit_cell = read(os.path.join(data_root, 'GaSe.cif'))
        elif materials_id == 'BP_spin_500':
            unit_cell = read(os.path.join(data_root, 'P.cif'))

        if materials_id in ['MoS2_500', 'WSe2_500', 'hBN_spin_500']:
            supercell_matrix = np.array([[8, 0, 0],
                                        [0, 8, 0],
                                        [0, 0, 1]])
        else:
            supercell_matrix = np.array([[6, 0, 0],
                                        [0, 6, 0],
                                        [0, 0, 1]])
